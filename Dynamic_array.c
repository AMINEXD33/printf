 ...
 *Description:if the new string that needs to be added to the BUFFER is over flowing , meaning there is
    no space for it , this function will reallocate a new BUFFER that can hold the wanted string and copy
    the old BUFFER to the new one will also add a marging of (10 bits), to save up time , in case next str
    can fit without allocating and copying the whole BUFFER.
 *@struct BUFF: is the structure of the current BUFF
 ...
 *Description: if the new string can be copied to the BUFFER , this function will copy it,in the other
 *hand if not , the function will pass so allocating and copying , SEE (re_allocate) above
 *@struct BUFF: the BUFFER struct
...
int array_push(struct D_array *BUFF,char *str)
{
    ...
    {
        //printf("must reallocate! and copy [%s]\n", str);
    ...
    {
        //printf("no need to allocate , coping the new str %s\n", str);
        y = 0;
        for(x = BUFF->index; str[y] != '\0'; x++)
        {
            BUFF->arr[x] = str[y];
